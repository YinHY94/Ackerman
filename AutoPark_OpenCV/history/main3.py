...
while cap.isOpened():
    ...
    cv2.drawContours(contour_img, contours, -1, 255, 1)  # 只绘制边缘，厚度为1
    
    # 3. 用霍夫变换检测边缘中的线段
    lines = cv2.HoughLinesP(contour_img, 1, np.pi / 180, threshold=40, minLineLength=30, maxLineGap=15)
    ...
    if lines is not None:
        ...
        for line in lines:
            ...
            if abs(angle) < 45:
                ...
                # 可视化检测到的水平线
                cv2.line(original_frame, (x1, y1), (x2, y2), (0, 255, 0), 1)
    ...
    if lane_lines:
        ...
        # 绘制检测到的平均线
        cv2.line(original_frame, (left_x, left_y), (right_x, right_y), (0, 0, 255), 2)
    ...
    # 显示边缘检测结果
    cv2.imshow("Contour Edges", contour_img)
